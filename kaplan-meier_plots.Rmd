...
# Create plotdata_clinical_ctdna_inclusion dataframe focusing on Midpoint HPV DNA
plotdata_clinical_ctdna_inclusion <- df_clinical_ctdna %>%
  ...
  mutate(
    ...
    midpoint_ctdna_level = if_else(`Mid RT` > median(`Mid RT`, na.rm = TRUE), 'Above Median', 'Below Median')
  )

# Extract some variables
val_smoking_status_by_hpv_detectability_midpoint <- chisq.test(
  x = plotdata_clinical_ctdna_inclusion$hpv_detectability_midpoint,
  y = plotdata_clinical_ctdna_inclusion$smoking_status
) %>% .$p.value %>% signif(2)

val_t_category_by_hpv_detectability_midpoint <- chisq.test(
  x = plotdata_clinical_ctdna_inclusion$hpv_detectability_midpoint,
...
survfit_rfs_midpoint_detectable <- survfit(
  surv_recurrence ~ midpoint_ctdna_level,
  data = pd_survival_by_midpoint
)
p_surv_rfs_midpoint_detectable <- ggsurvplot(survfit_rfs_midpoint_detectable, pval = T) + ggtitle('RFS by Midpoint HPV DNA Level')

survfit_pfs_midpoint_detectable <- survfit(
  surv_progression ~ midpoint_ctdna_level,
  data = pd_survival_by_midpoint
)
p_surv_pfs_midpoint_detectable <- ggsurvplot(survfit_pfs_midpoint_detectable, pval = T) + ggtitle('PFS by Midpoint HPV DNA Level')

survfit_os_midpoint_detectable <- survfit(
  surv_overall ~ midpoint_ctdna_level,
...
p_surv_os_midpoint_detectable <- ggsurvplot(survfit_os_midpoint_detectable, pval = T) + ggtitle('OS by Midpoint HPV DNA Level')
...
plot_grid(
  p_surv_rfs_midpoint_detectable$plot + theme(legend.position = 'none'),
  p_surv_pfs_midpoint_detectable$plot + theme(legend.position = 'none'),
  p_surv_os_midpoint_detectable$plot + theme(legend.position = 'none'),
  get_legend(
    p_surv_os_midpoint_detectable$plot + theme(legend.position = 'right')
  ),
