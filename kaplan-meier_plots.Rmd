...
# Create Strata by median 
pd_survival_by_baseline <- df_cbctfate %>%
  filter(fraction == 1) %>% 
  left_join(df_clinical_ctdna, by = "Mrn") %>%
  mutate(
    baseline_gtvp = if_else(smoothed > median(smoothed, na.rm = TRUE), 'Above Median', 'Below Median'),
    status_progression = ifelse(progression == TRUE, 1, 0)  # Create the status_progression column
  )

# Create survival object
surv_obj <- Surv(time = pd_survival_by_baseline$time_to_progression,
                 event = pd_survival_by_baseline$status_progression)

# Kaplan-Meier plot for Baseline
survfit_pfs_baseline_detectable <- survfit(
  surv_obj ~ baseline_gtvp,
  data = pd_survival_by_baseline
)
...
p_surv_pfs_baseline_detectable <- ggsurvplot(
  ...
  pval = TRUE,
  legend.labs = c("Above\nMedian", "Below\nMedian"),
  title = "PFS by Baseline Tumor Volume"
)
...
ggsave("../results/Baseline_GTV_KM.png", km1, width = 4, height = 3)
