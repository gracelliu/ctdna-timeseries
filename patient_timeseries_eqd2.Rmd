...
create_combined_plots <- function(df_cbctvolume, fraction_label, fill_color) {
  ...
  t_stage_plot <- df_cbctvolume %>%
    filter(Roi == 'x_GTVp_Tracking') %>%
    ggplot(aes(x = stage_t_ajcc8, y = percent_vol)) +
  ...
  n_stage_plot <- df_cbctvolume %>%
    filter(Roi == 'x_GTVp_Tracking') %>%
    ggplot(aes(x = stage_n_ajcc8, y = percent_vol)) +
    geom_boxplot(fill = fill_color) +
    labs(title = fraction_label, x = NULL, y = NULL) +
    theme_minimal()
...
# Fraction 1
fraction1_plots <- create_combined_plots(df_cbctvolume %>% filter(fraction == 1), "Fraction 1", "antiquewhite4")

# Fraction 7
fraction7_plots <- create_combined_plots(df_cbctvolume %>% filter(fraction == 7), "Fraction 7", "antiquewhite3")

# Fraction 16
fraction16_plots <- create_combined_plots(df_cbctvolume %>% filter(fraction == 16), "Fraction 16", "antiquewhite2")

# Combine Tumor Stage plots
t_combined_plot <- ggarrange(
  fraction1_plots$t_stage_plot, fraction7_plots$t_stage_plot, fraction16_plots$t_stage_plot,
  labels = NULL,
  ncol = 3, nrow = 1,
...
t_annotated <- annotate_figure(t_combined_plot, 
                               left = textGrob("Tumor Volume Change (%)", rot = 90, vjust = 0.2, gp = gpar(cex = 1.3)),
                               bottom = textGrob("Tumour Stage (T)", gp = gpar(cex = 1.3)))

# Combine Nodal Stage plots
n_combined_plot <- ggarrange(
  fraction1_plots$n_stage_plot, fraction7_plots$n_stage_plot, fraction16_plots$n_stage_plot,
  labels = NULL,
  ncol = 3, nrow = 1,
  common.legend = TRUE, legend = "bottom",
  align = "hv", 
...
ggsave("../results/tumor_volume_Tstage_plot.png", t_annotated, width = 12, height = 6)
ggsave("../results/tumor_volume_Nstage_plot.png", n_annotated, width = 12, height = 6)
