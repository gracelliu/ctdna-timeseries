...
```{r}
# Inspect the data types and unique identifiers
str(df_cbctvolume$patient)
str(df_hpvseq_dates$patient)

# Ensure there are no leading/trailing spaces and convert both to character
df_filtered_gtv <- df_filtered_gtv %>%
  mutate(Mrn = as.character(Mrn))

df_hpvseq_dates <- df_hpvseq_dates %>%
  mutate(patient = trimws(patient), patient = as.character(patient))
...
merged_data <- df_cbctvolume %>%
  left_join(overall_ctdna_change, by = c("patient"))
...
ggplot(merged_data, aes(x = percent_vol, y = seq_copies_mL_log)) +
  geom_point() +
  labs(title = "Scatter Plot of ctDNA vs. Tumor Volume",
       x = "Tumor Volume (percent_vol)",
       y = "ctDNA (log scale)")
