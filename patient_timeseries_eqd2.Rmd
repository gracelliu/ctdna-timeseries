...
create_patient_plot <- function(patient_id) {
  ...
  tumor_volume_plot <- df_cbctvolume_shared %>%
    ...
    geom_line(size=0.2) +
    labs(title = "Volume of Primary Tumor over the course of treatment",
         x = "Fraction",
         y = "Volume of Primary Tumor") +
    theme_minimal()
  ...
  air_volume_plot <- df_cbctvolume_shared %>%
    ...
    ggplot(aes(x = fraction, y = percent_vol)) +
    geom_line(size=0.2) +
    labs(title = "Air Volume in Primary Tumor over course of treatment",
         x = "Fraction",
         y = "Air Volume in Primary Tumor") +
    theme_minimal()
  ...
  ctdna_plot <- df_hpvseq_dates_shared %>%
    ...
    ggplot(aes(x = fraction, y = seq_copies_mL_log, group = patient)) +
    geom_line(size=0.1, color='gray30', na.rm = TRUE) + 
    geom_point(aes(color = timepoint), size = 1) +
    scale_color_manual(values = c(
      "baseline" = "#0072B2",
    ...
    labs(
      title = "HPV ctDNA Concentration over the course of treatment",
      x = "Fraction",
      y = "HPV ctDNA Concentration (log scale)"
    ) +
    theme_minimal()
  ...
  draw_label(patient_id, x = 0.1, y = 0.95, hjust = 0, vjust = 1, fontface = "bold", size = 10)
...
# Save each plot as a file or display them in RStudio
for (i in 1:length(shared_patients)) {
  ggsave(filename = paste0("patient_plot_", shared_patients[i], ".png"), plot = all_patient_plots[[i]], width = 8, height = 10)
}

# Display the first patient's plot as an example
print(all_patient_plots[[1]])
