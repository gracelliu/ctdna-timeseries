...
df_virus_frag = fread("/Users/lucaspenny/Desktop/fragment_length_summary.csv")
...
split_and_extract <- function(x,name) {
  split_1 <- unlist(strsplit(x, name))
  part_1 <- split_1[1]
...
# df_virus_frag$virus <- sapply(df_virus_frag$filename, function(x) split_and_extract(x,"_metrics.txt")[2])
df_virus_frag$filename <- sapply(df_virus_frag$filename, function(x) split_and_extract(x,"_metrics.txt")[1])
...
df_prop_virus <- df_virus_frag %>%
  # Calculate the sum of columns 3 through 603
  mutate(total_sum = rowSums(.[, 53:306])) %>%
  #filter(virus == "HPV16Aligned") %>% 
  dplyr::select(filename, everything()) %>%
  left_join(dplyr::select(df_OPC, filename, timepoint), by = "filename") %>% 
...
ggplot(df_summary, aes(x = length, y = median_proportion)) +
  geom_line() +
  geom_ribbon(aes(ymin = median_proportion - sd_proportion, ymax = median_proportion + sd_proportion), alpha = 0.2) +
...
# x = (df_OPC %>% dplyr::filter(!is.na(IntegrationHigh)) %>% dplyr::select(filename)) #change to what  want
# x = (df_OPC %>% dplyr::filter(IntegrationHigh>10) %>% dplyr::select(filename)) #change to what filter I
x = (df_clinical %>% dplyr::filter(stage_n_ajcc8 == "N2") %>% dplyr::select(filename))
df_fraglength_prop_long <- df_prop_virus %>%
...
columns_without_NaN <- colSums(is.nan(m_all_insertsize_extended)) == 0

# Subset m_all_insertsize_extended to keep only those columns
m_all_insertsize_extended <- m_all_insertsize_extended[, columns_without_NaN]
