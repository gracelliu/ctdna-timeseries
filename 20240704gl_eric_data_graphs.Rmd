...
df_filtered_gtv$overall_change <- factor(df_filtered_gtv$overall_change, levels = c("positive", "negative"))

df_filtered_gtv <- df_filtered_gtv %>%
  mutate(overall_change = recode(overall_change, 
                            "positive" = "Positive", 
                            "negative" = "Negative"))

# Create the plot with lines colored based on overall change
g1 <- ggplot(df_filtered_gtv, aes(x = fraction, y = percent_vol, group = Mrn, color = overall_change)) +
  geom_line(size = 0.2) +
  scale_color_manual(values = c("Positive" = "#FFB296", "Negative" = "#78BBC1")) +
  scale_x_continuous(
    breaks = scales::pretty_breaks(n = 10)
  ) +
  labs(x = "Fraction",
       y = "Percent Change in Tumour Volume",
       color = "Overall Change")

print(g1)
ggsave("../results/GTV_all.png", g1, width = 6.6, height = 3.3)
...
print(g2)
ggsave("../results/avg_tumour_all.png", g2, width = 5, height = 3)
...
ggsave("../results/ctdna_Tstage_plot.png", t_annotated, width = 7, height = 3)
ggsave("../results/ctdna_Nstage_plot.png", n_annotated, width = 7, height = 3)
...
# Initial data processing steps as provided
...
# Calculate log of seq_copies_mL
df_hpvseq_dates <- df_hpvseq_dates %>%
  mutate(seq_copies_mL_log = log1p(seq_copies_mL))
...
# Classify percent change at follow-up as positive or negative
ctdna_change_at_followup <- df_hpvseq_dates %>%
  filter(timepoint == "followup") %>%
  group_by(patient) %>%
  summarize(overall_change_ctdna = ifelse(percent_hpvseq > 0, "positive", "negative"))

# Convert `patient` column to character in both datasets
df_filtered_gtv <- df_filtered_gtv %>%
  mutate(Mrn = as.character(Mrn))

ctdna_change_at_followup <- ctdna_change_at_followup %>%
  mutate(patient = as.character(patient))

# Merge the datasets on the patient identifier
merged_data <- df_filtered_gtv %>%
  left_join(ctdna_change_at_followup, by = c("Mrn" = "patient"))
...
ggplot(merged_data, aes(x = reorder(Mrn, percent_vol), y = percent_vol, fill = overall_change_ctdna)) +
  ...
  labs(title = "Waterfall Plot of Tumour Volume Change",
       x = "Patient",
       y = "Percent Change in Tumour Volume",
...
```{r}
# count unique values in df_cbctfate 
unique_count_df_cbctfate <- df_cbctfate %>% 
  summarise(unique_count_df_cbctfate = n_distinct(Mrn))

unique_count_df_cbctfate
