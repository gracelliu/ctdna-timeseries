...
t_early <- ggplot(df_early_rt, aes(x = stage_t_ajcc8, y = seq_copies_mL_log)) +
  ...
  labs(title = "Early", x=NULL, y=NULL)
...
# Sequence copies by T Stage 
t_fu <- ggplot(df_3_month, aes(x = stage_t_ajcc8, y = seq_copies_mL_log)) +
...
# Sequence copies by N Stage 
n_fu <- ggplot(df_3_month, aes(x = stage_n_ajcc8, y = seq_copies_mL_log)) +
  geom_boxplot(fill="antiquewhite1") +
...
ggsave("../results/ctdna_Tstage_plot.png", t_annotated, width = 12, height = 6)
ggsave("../results/ctdna_Nstage_plot.png", n_annotated, width = 12, height = 6)
...
# Tumor Stage Plot
tumor_stage_plot <- ggplot(df_baseline, aes(x = stage_t_ajcc8, y = seq_copies_mL_log)) +
  geom_boxplot(fill="antiquewhite4") +
  labs(x = "", y = "HPV Sequence Copies (count per mL)", title = "Baseline") +
  theme_minimal() +
  theme(axis.title.x = element_blank()) +
  geom_boxplot(data = df_early_rt, fill = "antiquewhite3") +
  labs(title = "Early RT") +
  geom_boxplot(data = df_mid_rt, fill = "antiquewhite2") +
  labs(title = "Mid RT") +
  geom_boxplot(data = df_3_month, fill = "antiquewhite1") +
  labs(title = "3 Months Post-RT")

# Combine into a single plot layout
combined_tumor_plot <- (tumor_stage_plot + 
                        labs(x = "Tumor Stage (T)")) + 
                        plot_annotation(title = "HPV Sequence Copies by Tumor Stage")

print(combined_tumor_plot)
